            ...
            <table>
                ...
                <tbody>
                    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$table): $mod = ($i % 2 );++$i;?><tr>
                       		<form id="<?php echo ($table["name"]); ?>" method="post" action="<?php echo U('in');?>" enctype="multipart/form-data">
                			...
                			<input type="submit" value="导入表">
                			<input type="button" value="导出表" onclick="window.location.href='<?php echo U('out?tables='.$table['name']);?>'">
                			</form>
                            </td>
