...
class AuthGroupModel extends Model {
    ...
    protected $_validate = array(
       ...
       // array('rules','/^(\d,?)+(?<!,)$/', '规则数据不合法', Model::VALUE_VALIDATE , 'regex'  ,Model::MODEL_BOTH ),
        array('level','require', '必须设置角色等级', Model::MODEL_BOTH ),
    );
     ...
     */
    static public function getAuthExtend($uid,$session){
        if ( $session ) {
        ...
        $prefix = C('DB_PREFIX');
        /**
        *与原来的割裂开。原来是 班级权限和角色绑定，而现在班级权限与用户绑定
        */
        // $result = M()
        //     ->table($prefix.self::AUTH_GROUP_ACCESS.' g')
        //     ->join($prefix.self::AUTH_EXTEND.' c on g.uid=c.uid')
        //     ->where("g.uid='$uid' and c.type='$type' and !isnull(extend_id)")
        //     ->getfield('extend_id',true);
        $result = M()
            ->table($prefix.self::AUTH_EXTEND.' g')
            ->where("g.uid='$uid' and !isnull(extend_id)")
            ->getfield('extend_id',true);
        if ( $uid == UID && $session ) {
    ...
    static public function getAuthCategories($uid){
        return self::getAuthExtend($uid,'AUTH_CATEGORY');
    }
     ...
     * @author 朱亚杰 <xcoolcc@gmail.com>
     */
    static public function getExtendOfGroup($uid){
        return M(self::AUTH_EXTEND)->where( array('uid'=>$uid) )->getfield('extend_id',true);
    }
     ...
     * @author 朱亚杰 <zhuyajie@topthink.net>
     */
    static public function getClassOfGroup($uid){
        return self::getExtendOfGroup($uid);
    }
     ...
     * @author 朱亚杰 <xcoolcc@gmail.com>
     */
    static public function addToExtend($uid,$cid){
        $uid = is_array($uid)?implode(',',$uid):trim($uid,',');
     ...
     * @author 朱亚杰 <zhuyajie@topthink.net>
     */
    static public function addToClass($uid,$cid){
        return self::addToExtend($uid,$cid);
    }
     ...
     * @author 朱亚杰 <zhuyajie@topthink.net>
     */
    public function checkClassId($cid){
        return $this->checkId('Class',$cid, '以下分类id不存在:');
    }
