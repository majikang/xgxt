...
class ClassModel extends Model{
    ...
    protected $_validate = array(
        ...
        array('college', 'require', '所属学院不能为空', self::MUST_VALIDATE , 'regex', self::MODEL_BOTH),
    );
    ...
    public function getChildrenId($cate) {
        $field    = 'id,name,pid,title,college';
        $class = $this->getTree($cate, $field);
        $ids[]    = $cate;
        foreach ($class['_'] as $key => $value) {
            $ids[] = $value['id'];
        }
        return implode(',', $ids);
    }
